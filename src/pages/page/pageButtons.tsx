...
export const PageButtons: React.FC<Props>  = ({list_followers, owner, page_id}) => {
    const username = useSelector<RootState, UserState>(state => state.user).userInfo?.username
    if (username === owner){
        return(
            <div>
        ...
        return (
            <MyButton onClick={start_follow} children="follow" uuid={page_id} />
        )
    }
    else {
        return (
